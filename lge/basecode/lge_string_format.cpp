...
//       各种带符号的“整数”类型（包括char, short, int, 各种enum等。但不包括后来才定义的long long，即I64），在传递时全部转为long类型
//       各种无符号的“整数”类型与之类似，在传递时全部转为unsigned long类型
//       float类型在传递时全部转为double类型
//       后来定义的long long（VC6的I64）、unsigned long long（VC6的unsigned I64）、long double，在传递时按照原样传递，不转化
...
//    pStrOutput: 要输出的位置
// =====================================================================================================================
bool s_OutputSign(uint64 t, int iFlags, std::string *pStrOutput)
{
	if (!pStrOutput) {
		return false;
	}
...
//    pStrOutput: 要输出的位置
// =====================================================================================================================
bool s_OutputInteger_Unsigned(uint64 t,
							  int iRadix,
							  int iPrecision,
							  const char *pDigits,
...
public:
	...
	void AddArgument_Int64(int64 data);
...
// =====================================================================================================================
// =======================================================================================================================
void CStringFormatter::CImpl::AddArgument_Int64(int64 data)
{
	//~~~~~~~~~
	ARG_DATA arg;
	//~~~~~~~~~
	...
	case 'X':
		{
			...
			case 'o':
				if (iFlags & LONG64_INTEGER) {
					...
					if (!s_OutputInteger_Unsigned((uint64)(data >= 0 ? data : -data), iRadix, iPrecision,
						pDigits, &strBuf)) {
						s_RaiseError(InternalError, "Internal Error");	// we should never reach here
						return false;
					}
			...
			case 'X':
				if (iFlags & LONG64_INTEGER) {
					if (!s_OutputInteger_Unsigned((uint64)arg.int64Data, iRadix, iPrecision, pDigits, &strBuf
						)) {
						s_RaiseError(InternalError, "Internal Error");	// we should never reach here
						return false;
					}
...
// =====================================================================================================================
// =======================================================================================================================
void CStringFormatter::AddArgument(int64 data)
{
	if (!m_pImpl) {
		s_RaiseError(InternalError, "Internal Error");	// we should never reach here
		return;
...
// =====================================================================================================================
// =======================================================================================================================
void CStringFormatter::AddArgument(uint64 data)
{
	if (!m_pImpl) {
		s_RaiseError(InternalError, "Internal Error");	// we should never reach here
		return;
	...
	m_pImpl->AddArgument_Int64((int64) data);
}
...
// =====================================================================================================================
// =======================================================================================================================
CFormatHelper & CFormatHelper::operator << (int64 data)
{
	m_formatter.AddArgument(data);
	return *this;
}
...
// =====================================================================================================================
// =======================================================================================================================
CFormatHelper & CFormatHelper::operator << (uint64 data)
{
	m_formatter.AddArgument(data);
	return *this;
}
